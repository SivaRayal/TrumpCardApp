import { getUsedQuoteIndices } from './database';

// All quotes are bundled inline from the quotes.txt content
const QUOTES: string[] = [
  ...
  "Darkness is the absence of light; your purpose is to be the light.",
  "The secret of living well and longer is to eat half, walk double, laugh triple and love without measure.",
...
export async function pickRandomQuote(userId: number): Promise<{ quote: string; index: number }> {
  const used = await getUsedQuoteIndices(userId);

  let available = QUOTES
    .map((_, i) => i)
...
export async function getQuoteByIndex(index: number): Promise<string> {
